...
## Intramonth timing overlay

Per Nathan, Suominen & Tasa, "The Intramonth Momentum Cycle" (SSRN 6426026): in 1980-2025 US equity data, essentially all Winners-Minus-Losers momentum return comes from a six-trading-day window ending four days before month-end ($1→$18.78 in that window vs $1→$2.37 the rest of the month), driven by predictable month-end liquidity-driven selling of loser stocks.

This variant only opens new momentum-continuation entries during that identified six-day calendar window each month; outside the window it holds no new positions (existing positions still respect stop-loss/take-profit exits). This is a time-slice axis break from the parent's always-on daily rebalance, not a parameter retune.
