...
## Insider-cluster variant

Breaks two axes from the parent: (1) signal source — primary entry trigger is *multiple insiders buying within a short window* (cluster buying), with general news/filing summaries demoted to secondary confirmation only, rather than disclosure-news-driven daily scanning; (2) time-slice — weekly rebalance instead of daily, since cluster formation develops over days and daily re-scanning was diluting the signal (parent produced zero trades in 14 days despite having get_insider_trades access). Wider stop/take-profit (-5%/+8%) reflects the longer expected holding period for a cluster-driven thesis to play out.
