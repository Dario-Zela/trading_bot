...
## Regime-Conditional Signal Routing (HarbourfrontTechnologies, April 2026 + Meng & Chen arXiv 2605.23905)

This variant routes capital exclusively to **either mean-reversion mode OR momentum mode** each session based on a cross-asset regime assessment. **Do not blend both modes in the same session.**

### Step 1 — Regime Detection (assess before stock selection)

Use cross-asset signals; traditional technicals are secondary:

**RISK-OFF signals** (route to mean-reversion mode):
- Yield curve inverted or flattening sharply (US/UK 10y-2y spread < 0 or declining >15bp week-on-week)
- Gold/equity ratio rising (gold outperforming equities week-on-week)
- Credit spreads widening
- Bitcoin declining >5% over past 5 sessions (risk-appetite proxy per HarbourFront finding)

**RISK-ON signals** (route to momentum mode):
- Yield curve steep and stable/steepening (10y-2y > +0.75%)
- Gold/equity ratio stable or declining
- Credit spreads stable or tightening
- Bitcoin stable or rising

**Mixed / uncertain**: reduce position count to 1, require highest-conviction single name only.

### Step 2 — Mode Execution

**Mean-reversion mode (RISK-OFF):** Select stocks down >5% vs their sector over 5 days and trading below their 200-day SMA. Expect idiosyncratic mean-reversion toward sector mean. Wider stops appropriate (-4%) as volatility is elevated.

**Momentum mode (RISK-ON):** Select stocks with positive 3-month price momentum, above their 50-day SMA, in sectors with positive relative flow. Let winners run (TP = 5%).

**Key constraint:** commit to one mode for the full session. A regime assessment that produces one mean-reversion pick and one momentum pick is a signal of conflicting evidence — treat as mixed/uncertain and cut to 1 position maximum.

Sources: HarbourfrontTechnologies regime-aware ML study (April 2026, walk-forward Sharpe 1.18); Meng & Chen (arXiv 2605.23905, May 2026) on cross-asset features dominating traditional technicals for high-beta names.
